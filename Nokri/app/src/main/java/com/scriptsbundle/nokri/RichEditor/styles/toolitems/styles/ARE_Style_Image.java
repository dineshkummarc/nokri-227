...
import com.scriptsbundle.nokri.RichEditor.styles.IARE_Image;
...
public class ARE_Style_Image implements IARE_Style, IARE_Image {
	...
	public void insertImage(final Object src, final AreImageSpan.ImageType type) {
        ...
        if (type == AreImageSpan.ImageType.URI) {
            Glide.with(mContext).load((Uri) src).into(myTarget);
        } else if (type == AreImageSpan.ImageType.URL) {
        	    Glide.with(mContext).load((String) src).into(myTarget);
